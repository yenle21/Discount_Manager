...
def test_apply_voucher_no_cart(sample_vouchers,test_client):
    with test_client.session_transaction() as sess:
        sess['cart'] = {}
    payload = {
        "voucher_id": sample_vouchers[0].MaGG
    }
    response = test_client.put(f'/api/apply-voucher/{sample_vouchers[0].MaGG}', json=payload)
...
def test_apply_voucher_success(test_client, test_session, sample_vouchers):
    sample_vouchers[0].Hinhthuc = "Promotion"
    test_session.commit()
    with test_client.session_transaction() as sess:
        sess['cart'] = {
            "1": {
    ...
    assert data['applied_vouchers']['PROMOTION']['MaGG'] == sample_vouchers[0].MaGG

# Test chưa đủ tiền tối thiểu
def test_apply_voucher_insufficient_total(test_client, test_session, sample_vouchers):
    with test_client.session_transaction() as sess:
        sess['cart'] = {
            "1": {
                "id": "1",
    ...
    res = test_client.put(f'/api/apply-voucher/{sample_vouchers[0].MaGG}',
                          json={"voucher_id": sample_vouchers[0].MaGG})
    data = res.get_json()
    assert data['status'] == 404
...
def test_apply_voucher_wrong_category(test_client, test_session, sample_vouchers):
...
def test_apply_voucher_mixed_cart_success(test_client, test_session, sample_vouchers):
    sample_vouchers[0].DieuKienSP = "1"
    test_session.commit()
...
def test_apply_voucher_upcoming(test_client, test_session, sample_vouchers):
    sample_vouchers[0].NgayBD = datetime.now() + timedelta(days=1)
    test_session.commit()
    ...
    response = test_client.put(f'/api/apply-voucher/{sample_vouchers[0].MaGG}',
                               json={"voucher_id": sample_vouchers[0].MaGG})
...
# Kiem tra ma het han
def test_apply_voucher_expired(test_client, test_session, sample_vouchers):
    sample_vouchers[0].NgayKT = datetime.now() - timedelta(days=30)
    test_session.commit()
...
def test_apply_voucher_out_of_stock(test_client, test_session, sample_vouchers):
    sample_vouchers[0].SoLuong = 10
    sample_vouchers[0].DaSuDung = 10
    test_session.commit()
...
def test_apply_voucher_overwrite_existing(test_client, test_session, sample_vouchers):
    sample_vouchers[0].Hinhthuc = "Promotion"
    test_session.commit()
...
def test_delete_voucher_success(test_client, test_session):
    with test_client.session_transaction() as sess:
...
def test_update_voucher_success(test_session, sample_vouchers):
    new_data = {
        'Hinhthuc': 'Promotion',
        'LoaiGG': 'PERCENTAGE',
        'GiaTri': 20,
    ...
    # 2. Gọi hàm update
    result = dao.update_voucher(sample_vouchers[0].MaGG, new_data)

    # 3. Kiểm tra
    assert result is True
    assert sample_vouchers[0].GiaTri == 20
    assert sample_vouchers[0].SoLuong == 50